...
async function getDashboardSupabaseClient(): Promise<DashboardSupabaseClient | null> {
  ...
  try {
    ...
    console.error("[dashboard] Unable to initialize Supabase client:", error);
    return null;
  }
}
...
async function runDashboardQuery<T>(
  name: string,
  fallback: T,
  query: (supabase: DashboardSupabaseClient) => Promise<T>,
): Promise<T> {
...
export async function getDashboardKPIs(dateRange?: DashboardDateRange): Promise<DashboardKPIs> {
  return runDashboardQuery("getDashboardKPIs", getEmptyDashboardKPIs(), async (supabase) => {
    const now = new Date();
    ...
    const transferAmount =
      currentPayments?.filter((p) => p.method === "transfer").reduce((sum, p) => sum + Number(p.amount_paid || 0), 0) || 0;
...
export async function getRevenueByMonth(): Promise<RevenueByMonth[]> {
  return runDashboardQuery("getRevenueByMonth", getEmptyRevenueByMonth(), async (supabase) => {
...
export async function getPlanDistribution(): Promise<PlanDistribution[]> {
  return runDashboardQuery("getPlanDistribution", [] as PlanDistribution[], async (supabase) => {
...
export async function getRecentPayments(limit: number = 10): Promise<RecentPayment[]> {
  return runDashboardQuery("getRecentPayments", [] as RecentPayment[], async (supabase) => {
    const { data } = await runPaymentsPostedQueryCompat((usePostedFilter) => {
      let query = supabase
        .from("payments")
        .select(
...
export async function getExpiringSubscriptions(daysAhead: number = 5): Promise<ExpiringSubscription[]> {
  return runDashboardQuery("getExpiringSubscriptions", [] as ExpiringSubscription[], async (supabase) => {
...
export async function getInactiveCustomers(limit: number = 10): Promise<InactiveCustomer[]> {
  return runDashboardQuery("getInactiveCustomers", [] as InactiveCustomer[], async (supabase) => {
    const today = new Date();
...
export async function getSubscriptionsFlow(): Promise<SubscriptionsFlow[]> {
  return runDashboardQuery("getSubscriptionsFlow", getEmptySubscriptionsFlow(), async (supabase) => {
...
export async function getPaymentMethodDistribution(
  ...
  return runDashboardQuery("getPaymentMethodDistribution", [] as PaymentMethodDistribution[], async (supabase) => {
    const now = new Date();
